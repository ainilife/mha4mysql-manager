...
"Got MySQL error $mysql_err, but this is not mysql crash. Continue health check.."
          ...
          if ( $error_count >= 3 ) {
            ...
            $node_is_down = 1 if ( $self->is_secondary_down() );
            last if ($node_is_down);
            $error_count = 0;
          }
          next;
        }
      ...
      eval {
        if ( $is_master == 1 ){
            ...
            $log->info("doing mysql ping $self->{ping_type}");
            if ( $self->{ping_type} eq $MHA::ManagerConst::PING_TYPE_CONNECT ) {
              $child_exit_code = $self->fork_exec( sub { $self->ping_connect() },
                "MySQL Ping" );
            }
  ...
  if ($@) {
    my $msg = "Got error when monitoring node: $@";
    $log->warning($msg);
    undef $@;
    return 2 if ( $self->{_already_monitored} );
    return 1;
  }
  return 1 unless ($node_is_down);
