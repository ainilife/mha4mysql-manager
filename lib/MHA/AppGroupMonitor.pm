      ...
      foreach my $server_to_check (@alive_servers){
          ...
          eval {
            ...
            $server_ping = new MHA::HealthCheckOnce(
              ...
              ssh_host       => $server_to_check->{ssh_host},
              ssh_ip         => $server_to_check->{ssh_ip},
              ssh_port       => $server_to_check->{ssh_port},
              ssh_connection_timeout => $server_to_check->{ssh_connection_timeout},
        ...
        "secondary_check_script is not defined. It is highly recommended setting it to check master reachability from two or more routes."
            ...
            my $is_master=0;
            if($server_to_check->{ip} eq $current_master->{ip} && $server_to_check->{port} eq $current_master->{port} ){
                $is_master=1;
            }
            ( $ret, $ssh_reachable ) = $server_ping->check_server_reachable($is_master);
            if ( $ret eq '2' ) {
              $log->error(
